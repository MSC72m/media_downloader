...
class TestUTF8SubprocessHandling:
    ...
    @pytest.mark.skip(
        reason="Test expects subprocess calls but ServiceController uses yt-dlp Python API"
    )
    def test_service_controller_encoding_environment(self):
        ...
        with patch("core.service_controller.subprocess.run") as mock_subprocess:
            ...
            assert "env" in call_kwargs, (
                "subprocess.run should be called with env parameter"
            )
            env = call_kwargs["env"]
            ...
            assert env["LC_CTYPE"] == "en_US.UTF-8", (
                "LC_CTYPE should be set to en_US.UTF-8"
            )
            assert env["PYTHONLEGACYWINDOWSSTDIO"] == "0", (
                "PYTHONLEGACYWINDOWSSTDIO should be set to 0"
            )

            # Check encoding parameters
            assert call_kwargs["encoding"] == "utf-8", "Encoding should be utf-8"
            assert call_kwargs["errors"] == "replace", (
                "Error handling should be 'replace'"
            )
            assert call_kwargs["text"] is True, "Text parameter should be True"

    @pytest.mark.skip(
        reason="Test expects subprocess calls but implementation uses yt-dlp Python API"
    )
    def test_metadata_service_encoding_environment(self):
        ...
        # Mock subprocess.run to capture environment
        with patch(
            "services.youtube.metadata_service.subprocess.run"
        ) as mock_subprocess:
            mock_result = Mock()
            mock_result.returncode = 0
    ...
    def test_unicode_decode_error_fallback(self):
        ...
        with patch("core.service_controller.subprocess.run") as mock_subprocess:
            ...
            mock_subprocess.side_effect = [
                UnicodeDecodeError(
                    "utf-8", b"problematic bytes", 0, 1, "invalid start byte"
                ),
                mock_result,
            ]
            ...
            controller._download_worker(
                download, "~/Downloads", None, completion_callback
            )

            # Verify subprocess was called twice (original + fallback)
            assert mock_subprocess.call_count == 2, (
                "Should call subprocess twice (original + fallback)"
            )

            # Check that fallback used latin-1 encoding
            fallback_call = mock_subprocess.call_args_list[1]
            fallback_kwargs = fallback_call[1]
            assert fallback_kwargs["encoding"] == "latin-1", (
                "Fallback should use latin-1 encoding"
            )
    ...
    def test_yt_dlp_command_encoding_options(self):
        ...
        with patch("core.service_controller.subprocess.run") as mock_subprocess:
            ...
            assert "--no-check-certificate" in cmd, (
                "Should include --no-check-certificate"
            )
    ...
    def test_original_error_scenario_handling(self):
        ...
        with patch("core.service_controller.subprocess.run") as mock_subprocess:
            ...
            assert "0xb0" in args[1], (
                "Error message should contain the problematic byte reference"
            )
    ...
    def test_utf8_error_fallback_to_ios_client(self):
        ...
        with patch("core.service_controller.subprocess.run") as mock_subprocess:
            ...
            assert mock_subprocess.call_count == 2, (
                "Should call subprocess twice (original + iOS client)"
            )

            # Check that iOS client was used
            ios_call = mock_subprocess.call_args_list[1]
            ios_cmd = ios_call[0][0]
            assert "youtube:player_client=ios" in ios_cmd, (
                "Should use iOS client in fallback"
            )
    ...
    def test_metadata_service_subprocess_encoding_parameters(self):
        ...
        with patch(
            ...
            assert "text" in call_kwargs, (
                "subprocess.run should be called with text parameter"
            )
            assert call_kwargs["text"] is True, (
                f"Expected text=True, got {call_kwargs.get('text')}"
            )
