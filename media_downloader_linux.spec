...
"""PyInstaller spec file for Media Downloader (Linux).

Build with:
    pyinstaller media_downloader_linux.spec

This produces:
    dist/MediaDownloader/  (folder with executable and all dependencies)

Architecture:
    - Python interpreter + all packages bundled in _internal/
    - ffmpeg: downloaded on first run or via scripts/download_ffmpeg_linux.py
    - Chromium browser: downloaded on first launch for cookie generation
"""
...
# ffmpeg is downloaded on first run (not bundled)
...
a = Analysis(
    ...
    hiddenimports=[
        ...
        "yt_dlp",
        "instaloader",
        # Playwright Python package (browser binary downloaded on first launch)
        "playwright",
...
print("[PyInstaller] Linux build complete")
