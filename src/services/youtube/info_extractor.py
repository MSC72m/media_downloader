...
class YouTubeInfoExtractor:
    ...
    def _extract_with_cookies(
        ...
        logger.info(f"[INFO_EXTRACTOR] Attempting extraction with cookies (client: {client_used}, cookie_file: {opts.get('cookiefile', 'None')})")

        try:
            with yt_dlp.YoutubeDL(opts) as ydl:  # type: ignore
                info = ydl.extract_info(url, download=False)
                if info:
                    logger.info(f"[INFO_EXTRACTOR] Successfully extracted info with cookies (client: {client_used})")
                    return info
        except yt_dlp.utils.DownloadError as e:
            error_msg = str(e)
            logger.warning(f"[INFO_EXTRACTOR] Download error with {client_used} client: {error_msg[:200]}")
            if self._is_cookie_error(error_msg):
                logger.warning(f"[INFO_EXTRACTOR] Cookie error detected with {client_used} - cookies may be invalid or expired")
        except Exception as e:
            logger.warning(f"[INFO_EXTRACTOR] Extraction error with {client_used}: {e}", exc_info=True)
    ...
    def _build_options(
        ...
        if cookie_path:
            ...
            if cookie_file.exists():
                ...
                logger.info(f"[INFO_EXTRACTOR] Using cookie file: {cookie_path} (size: {file_size} bytes)")
            else:
                logger.warning(f"[INFO_EXTRACTOR] Cookie file does not exist: {cookie_path}")
        elif browser:
        ...
        logger.debug(f"[INFO_EXTRACTOR] Built options with client: {client_type}, cookie_file: {opts.get('cookiefile', 'None')}")
